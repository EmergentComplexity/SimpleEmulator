...
// THe code for the 4 bit low level emulator:
// This code is meant to simulate the hardware function of specific ICs rather than to simply execute the code as the high level emulators do

void MainWindow::ClockStart2() {
    int freq = (1.00/ui->ClockEdit_2->text().toFloat() ) * (float)1000;
    timer2->start(freq /2);
    //ui->CLKEdit->setText( "hi");

}
void MainWindow::ClockStop2() {
    timer2->stop();
}

// Everything that the emulated computer does is here
void MainWindow::AutoClock2() {
    clk++;
    if (clk > 1) {
        clk = 0;
    }
   ...
   // loads the next porgram. this part of the code does not specifically represent any hardware
